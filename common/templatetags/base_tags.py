...
def contains_title(streamfield):
    return bool(
        [
            x
            for x in streamfield
            if x.block_type == "h2" or x.block_type == "h3" or x.block_type == "h4"
        ]
    )
...
def footer(context):
    ...
    league = (
        GenericPage.objects.live().child_of(root).filter(title__iexact="league").first()
    )
    league_subpages = GenericPage.objects.live().child_of(league).all()
    records = (
        GenericPage.objects.live()
        .child_of(root)
        .filter(title__iexact="records")
        .first()
    )
...
def navigation(context, calling_page=None):
    ...
    return {
        "is_home": (calling_page.url == "/" if calling_page else False),
...
def filters_to_str(tag, date):
    ...
    if date:
        ...
        month = int(month)
        filter_str += " from {month} {year}".format(
            month=to_month_str(month), year=year
        )
